...
public class MemberControl {
	...
	@ResponseBody
	public Object signup(Member member, Model model)
			throws Exception {
		JsonResult jsonResult = new JsonResult();
		try {
			jsonResult.setData(memberService.signUp(member));
	...
	public Object list() throws Exception {
		...
		try {
			jsonResult.setData(memberService.getMemberList());
			jsonResult.setStatus("success");
		} catch (Throwable e) {
			StringWriter out = new StringWriter();
			e.printStackTrace(new PrintWriter(out));
	...
	@ResponseBody
	public Object add(Member member, MultipartFile photo) throws Exception {
	...
	@ResponseBody
	public Object changePassword(String email,
			@RequestParam("password") String currPassword, String newPassword,
			String newPassword2, Model model) throws Exception {
	...
	@ResponseBody
	public Object update(Member member, MultipartFile photo) throws Exception {
		String filename = this.getNewFileName();
		String path = sc.getAttribute("rootRealPath") + "file/" + filename;
		photo.transferTo(new File(path));
		member.setPhotos(new String[] { filename });
