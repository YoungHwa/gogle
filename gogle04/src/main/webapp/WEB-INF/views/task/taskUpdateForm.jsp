...
<head>
<meta charset="UTF-8" />
<title>Gogle - Project Task Update</title>
			...
			<div class="box round first fullpage">
				...
				<div class="block ">
					<form action="update.do" method="post"
						...
						<input type="hidden" name="oldUiProtoUrl"
							value="${task.uiProtoUrl}">
						<table class="form">
							<tr>
							...
							<tr>
								...
								<td><input type="text" class="mini" name="title"
									value="${task.title}"></td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td><input type="file" class="mini" name="uiProto"></td>
							</tr>
							<tr>
								<td><label> 내용</label></td>
								<td><input type="text" class="mini" name="content"
									value="${task.content}"></td>
							</tr>
							<tr>
								<td><label> 시작일</label></td>
								<td><input type="text" class="mini" name="startDate"
									value="${task.startDate}" placeholder="예)2013-4-5"></td>
							</tr>
							<tr>
								<td><label> 종료일</label></td>
								<td><input type="text" class="mini" name="endDate"
									value="${task.endDate}" placeholder="예)2013-6-5"></td>
							</tr>
							<tr>
								<td><label> 상태</label></td>
								<td><select name="status">
										<option value="0"
											<c:if test="${task.status == 0}">selected</c:if>>등록</option>
										<option value="1"
											<c:if test="${task.status == 1}">selected</c:if>>진행</option>
										<option value="2"
											<c:if test="${task.status == 2}">selected</c:if>>완료</option>
										<option value="3"
											<c:if test="${task.status == 3}">selected</c:if>>지연</option>
								</select></td>
							</tr>
						</table>
